...
fun main() = application {
    ...
    program {
        ...
        val noise = worley2DT(-2.0, 2.0)
            ...
            .warp(
                Vector3.identity +
                        fastSimplex3D().vector3Range().bindSeed(200).scaleDomain(1.0) * Vector3(0.0, 0.0, 0.5).constant()
            )


        val f =
            noise.abs().colorMap(listOf(
                ColorMap(ColorRGBa.LIGHT_SKY_BLUE.toLinear(), 0.0, 0.1, 1.0),
                ColorMap(ColorRGBa.LIGHT_CORAL.toLinear(), 0.25, 0.1, 1.0),
                ColorMap(ColorRGBa.PINK.toLinear(), 0.5, 0.1, 1.0),
                ColorMap(ColorRGBa.LIGHT_GREEN.toLinear(), 0.75, 0.1, 1.0),
                ColorMap(ColorRGBa.LIGHT_SKY_BLUE.toLinear(), 1.0, 0.1, 1.0),
            ))
                .temporalBlur2DT(30,0.01)
